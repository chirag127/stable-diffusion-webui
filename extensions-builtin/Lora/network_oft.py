...
class NetworkModuleOFT(network.NetworkModule):
    def __init__(self, net: network.Network, weights: network.NetworkWeights):
        ...
        is_other_linear = type(self.sd_module) in [
        ...
        if self.rescale is not None and not is_other_linear:
            self.rescale = self.rescale.reshape(
                -1, *[1] * (self.org_module[0].weight.dim() - 1)
            )
    ...
    def calc_updown(self, orig_weight):
        ...
        if not self.is_R:
            block_Q = oft_blocks - oft_blocks.transpose(
                -1, -2
            )  # ensure skew-symmetric orthogonal matrix
            if self.constraint != 0:
                norm_Q = torch.norm(block_Q.flatten())
                new_norm_Q = torch.clamp(
                    norm_Q, max=self.constraint.to(oft_blocks.device)
                )
        ...
        updown = merged_weight.to(orig_weight.device) - orig_weight.to(
            merged_weight.dtype
        )
