...
from modules import (devices, errors, patches, script_callbacks,
                     sd_hijack_clip, sd_hijack_open_clip,
                     sd_hijack_optimizations, sd_hijack_unet, sd_hijack_xlmr,
                     sd_unet, shared, xlmr, xlmr_m18)
        ...
        if conditioner:
            for i in range(len(conditioner.embedders)):
                ...
                if isinstance(
                    ...
                    embedder.wrapped.transformer.text_model.embeddings.token_embedding = (
                        embedder.wrapped.transformer.text_model.embeddings.token_embedding.wrapped
                    )
                    conditioner.embedders[i] = embedder.wrapped
